...
- 每个ONU使用一个单独的波长，每个ONU能在共享的上行光纤中连续发送信息，而不用等待特定的预设时隙
- **优点**
...
- **ONU光源**
  ...
  - [**无色化ONU**](#无色化ONU方案)
    ...
    - **光谱分割方案**
      - ONU的发送机中包含宽谱光源（超辐射发光二极管 SLED）
    - **注入锁定方案**
      - CO使用宽带光源（宽带EDFA光源），OLT和ONU使用波长锁定FP激光器

    - **再调制方案**
      - 反射型半导体光放大器（RSOA）或反射型电吸收调制器在ONU上产生特有波长的源，用来传输上行数据流
